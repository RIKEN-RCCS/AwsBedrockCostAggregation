...
使い方:
    python bedrock_cost_report.py --bucket bedrock-invocation-logs-xxx \
                                  --year 2026 --month 5 \
...
PRICING = {
    ...
    # Claude Opus 4.5（既存）
    "anthropic.claude-opus-4-5-20250805-v1:0": {
        "input": Decimal("0.005"),
        "output": Decimal("0.025"),
    },
    ...
    "anthropic.claude-haiku-4-5-20250929-v1:0": {
        "input": Decimal("0.0008"),   # $0.8 / 1M tokens（互換性維持）
        "output": Decimal("0.004"),   # $4 / 1M tokens（互換性維持）
    },
    "apac.anthropic.claude-haiku-4-5-20250929-v1:0": {
        "input": Decimal("0.0008"),
        "output": Decimal("0.004"),
    },
    # Claude 3.5 Haiku（レガシー）
...
USD_TO_JPY = Decimal("155")  # 為替レート（要適宜更新）
...
# Bedrock実装では Regional endpoint (+10%) が適用されることが多い
# デフォルトは 5分キャッシュ（Claude 4.5以降のBedrock実装）
CACHE_ASSUME_5MIN = True  # False で 1時間キャッシュに変更
...
def load_stats(conn: sqlite3.Connection, year: int, month: int, owner_map=None):
    ...
    cur = conn.execute(
        ...
        SELECT identity_arn, username, model_id,
               ...
               SUM(CASE WHEN cache_ttl_type='5min' THEN cache_write_input_tokens ELSE 0 END),
...
def generate_report(stats, year: int, month: int) -> str:
    """Markdownレポート生成"""
    lines = []
    lines.append(f"# Bedrock利用レポート: {year}年{month}月")
    lines.append("")
...
def main():
    ...
    parser.add_argument("--bucket", required=True, help="ログ出力先S3バケット名")
    parser.add_argument("--year", type=int, required=True, help="集計対象年")
    parser.add_argument("--month", type=int, required=True, help="集計対象月")
    parser.add_argument("--region", default="ap-northeast-1", help="リージョン")
    parser.add_argument("--output", default=None, help="出力ファイル名(.md)")
    parser.add_argument("--db", default=str(DEFAULT_DB_PATH),
                        help=f"SQLite DBパス (default: {DEFAULT_DB_PATH})")
    parser.add_argument("--skip-ingest", action="store_true",
                        help="S3取り込みをスキップしDBの内容のみでレポート生成")
    parser.add_argument("--no-resolve-owner", action="store_true",
                        help="IAMタグによる所有者名解決を無効化")
    ...
    try:
        if not args.skip_ingest:
            ingest_logs(conn, args.bucket, args.year, args.month, args.region)

        owner_map = None
        if not args.no_resolve_owner:
            try:
        ...
        stats = load_stats(conn, args.year, args.month, owner_map)
        if not stats:
            print(f"⚠️  {args.year}年{args.month}月のデータがDBにありません")
            return

        report = generate_report(stats, args.year, args.month)
